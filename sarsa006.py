...
class NaiveAgent():
    ...
    def __createStateDict(self):
        ...
        for player_tan in range(-33, 44):
            for next_player_tan in range(-33, 44):
                state_key += 1
                # state[state_key] = { 'player_tan' : player_tan, 'player_tan_top' : player_tan_top }
                state[state_key] = { 'player_tan' : player_tan, 'next_player_tan': next_player_tan }
...
def sarsa():
    ...
    for episode in range(starting_episode,episodes):
        ...
        while(not p.game_over()):
            ...
            Target = reward + discount_factor * Q[S_prime][A_prime]
            Delta = Target - Q[S][A]
        ...
        with open("q-table.txt", "w") as file:
